...
export default function Pricing() {
  return (
    <section id="pricing" className="relative py-24 bg-zinc-50 dark:bg-zinc-950 overflow-hidden">
      <div className="pointer-events-none absolute inset-x-0 -top-24 h-48 bg-gradient-to-b from-indigo-500/10 via-violet-500/10 to-transparent blur-2xl" />
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {tiers.map((t, i) => (
            <motion.div
              ...
              className={`group relative rounded-2xl p-6 border ${t.highlight ? 'bg-white dark:bg-zinc-900 border-indigo-500 shadow-[0_0_0_1px_rgba(99,102,241,0.4)]' : 'bg-white/60 dark:bg-zinc-900/40 border-zinc-200 dark:border-zinc-800'}`}
            >
              {t.highlight && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 text-[10px] px-2 py-0.5 rounded-full bg-indigo-600 text-white border border-indigo-400 shadow-[0_0_24px_rgba(99,102,241,0.45)]">Most Popular</span>
              )}
              ...
              <button className={`relative mt-6 w-full py-2 rounded-xl font-medium overflow-hidden ${t.highlight ? 'bg-indigo-600 text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-900 dark:text-white'}`}>
                ...
                <span className="relative">{t.highlight ? 'Get Pro' : 'Choose plan'}</span>
              </button>
            </motion.div>
          ))}
        </div>
